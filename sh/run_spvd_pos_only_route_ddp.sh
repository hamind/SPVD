...
NAME="spvd_pos_only_route"
CONFIG="$ROOT/configs/experiments/spvd_pos_only_route.yaml"
...
PIP_BIN="${PIP_BIN:-/root/miniconda3/envs/openclip/bin/pip}"
...
if ! "$PYTHON_BIN" -c "import webdataset" >/dev/null 2>&1; then
  echo "[$NAME] installing missing dependency: webdataset==1.0.2"
  "$PIP_BIN" install webdataset==1.0.2
fi
