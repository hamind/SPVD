...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--use-route-loss", action=argparse.BooleanOptionalAction, default=bool(_default_loss_value(config, "use_route_loss", True)))
    parser.add_argument("--route-positive-constraint", action=argparse.BooleanOptionalAction, default=bool(_default_loss_value(config, "route_positive_constraint", True)))
    parser.add_argument("--route-negative-constraint", action=argparse.BooleanOptionalAction, default=bool(_default_loss_value(config, "route_negative_constraint", True)))
    parser.add_argument("--decomp-loss-weight", type=float, default=_default_loss_value(config, "decomp_loss_weight", _default_loss_value(config, "route_weight", 0.1)))
    parser.add_argument("--residual-loss-weight", type=float, default=_default_loss_value(config, "residual_loss_weight", 0.05))
    parser.add_argument("--orth-loss-weight", type=float, default=_default_loss_value(config, "orth_loss_weight", 0.01))
    parser.add_argument("--detach-relevance", action=argparse.BooleanOptionalAction, default=bool(_default_loss_value(config, "detach_relevance", True)))
