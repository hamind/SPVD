...
def create_loss(args: object) -> nn.Module:
    ...
    if loss_name == "spvd":
        return SPVDLoss(
            ...
            caption_loss_impl=getattr(args, "caption_loss_impl", "masked_sigmoid"),
            caption_mask_same_image=bool(getattr(args, "caption_mask_same_image", True)),
            caption_same_image_mode=getattr(args, "caption_same_image_mode", "ignore"),
            caption_region_pos_weight=float(getattr(args, "caption_region_pos_weight", 0.05)),
            caption_region_neg_weight=float(getattr(args, "caption_region_neg_weight", 0.02)),
            caption_region_start_weight=float(getattr(args, "caption_region_start_weight", 0.0)),
            caption_region_warmup_steps=int(getattr(args, "caption_region_warmup_steps", 0)),
            caption_region_pos_min_overlap=float(getattr(args, "caption_region_pos_min_overlap", 0.3)),
            caption_region_neg_max_overlap=float(getattr(args, "caption_region_neg_max_overlap", 0.1)),
            caption_region_overlap_gamma=float(getattr(args, "caption_region_overlap_gamma", 1.0)),
            caption_region_detach_overlap=bool(getattr(args, "caption_region_detach_overlap", True)),
            branch_bce_start_weight=float(getattr(args, "branch_bce_start_weight", 0.0)),
            residual_variance_start_weight=float(getattr(args, "residual_variance_start_weight", 0.0)),
            branch_bce_warmup_steps=int(getattr(args, "branch_bce_warmup_steps", 0)),
            residual_variance_warmup_steps=int(getattr(args, "residual_variance_warmup_steps", 0)),
